...
sync_repo() {
  ...
  if ! git diff --quiet --ignore-submodules -- || ! git diff --cached --quiet --ignore-submodules --; then
    [ "$sync_mode" = "strict" ] && fail "repository has local tracked changes; commit or stash them before running start"
  ...
  if [ -z "$current_branch" ]; then
    [ "$sync_mode" = "strict" ] && fail "repository is in detached HEAD state; switch to a branch before running start"
