...
- Current phase: Phase 8 - Admin Studio v1
...
- 既然公开站与后台都已经基于 `HeroUI React + Tailwind` 收口，后续 UI 必须继续复用共享组件和尺寸体系；不能在页面层重新长出另一套不一致的按钮、表单、卡片和 tabs 视觉。
...
Run `$gsd-verify-work 8` to verify the current Studio flow, or `$gsd-plan-phase 6` if you are switching back to the public discovery redesign.
