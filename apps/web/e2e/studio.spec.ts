...
test.describe('studio surface', () => {
	test('admin can log in and move across the new studio shell', async ({ page }, testInfo) => {
	...
	test('editor exposes save, validation, and license fields', async ({ page }, testInfo) => {
		await loginToStudio(page);
		await page.goto('/studio/snippets/basic-card-feed');
		...
		await expect(page.getByText(/校验通过|校验未通过/)).toBeVisible();
